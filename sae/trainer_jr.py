...
from .config import JRTrainConfig
...
class JRSaeTrainer:
    def __init__(
        self, cfg: JRTrainConfig, dataset: HfDataset | MemmapDataset, model: PreTrainedModel
    ):
        if cfg.hookpoints:
        ...
        self.model = model
        self.saes = {
            hook: JRSae(input_widths[hook], cfg.sae, device)
    ...
    def fit(self):
        ...
        avg_real_l0_loss = defaultdict(float)
        avg_l0_loss = defaultdict(float)

        hidden_dict: dict[str, Tensor] = {}
        name_to_module = {
            name: self.model.get_submodule(name) for name in self.cfg.hookpoints
        }
        maybe_wrapped: dict[str, DDP] | dict[str, JRSae] = {}
        ...
        for batch in dl:
            ...
            for name, hiddens in hidden_dict.items():
                ...
                for chunk in hiddens.chunk(self.cfg.micro_acc_steps):
                    ...
                    avg_l0_loss[name] += float(
                        self.maybe_all_reduce(out.l0_loss[1].detach()) / denom
                    )
                    avg_real_l0_loss[name] += float(
                        self.maybe_all_reduce(out.l0_loss[0].detach()) / denom
                    )
                    if self.cfg.auxk_alpha > 0:
                    ...
                    loss = out.fvu + self.cfg.auxk_alpha * out.auxk_loss + self.cfg.l0_coef * out.l0_loss[1]
                    loss.div(acc_steps).backward()

                    # Update the did_fire mask
                    fire_mask = out.latent_acts.abs().sum(0) > 0
                    did_fire[name] = did_fire[name] | fire_mask
                        ...
                        info.update(
                            {
                                ...
                                f"l1_loss/{name}": avg_l1_loss[name],
                                f"l0_loss/{name}": avg_l0_loss[name],
                                f"real_l0_loss/{name}": avg_real_l0_loss[name],
                            }
                        )
                        if self.cfg.auxk_alpha > 0:
